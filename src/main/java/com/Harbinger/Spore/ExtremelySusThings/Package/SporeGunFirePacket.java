...
public record SporeGunFirePacket(int id) {
   ...
   public static void handle(SporeGunFirePacket message, Supplier context) {
      ((NetworkEvent.Context)context.get()).enqueueWork(() -> {
         Player sender = ((NetworkEvent.Context)context.get()).getSender();
         if (sender != null) {
            ...
            if (!level.isClientSide) {
               ...
               if (truePlayer instanceof ServerPlayer) {
                  ...
                  Item patt1621$temp = stack.getItem();
                  if (patt1621$temp instanceof AbstractSporeGun) {
                     AbstractSporeGun gun = (AbstractSporeGun)patt1621$temp;
                     Vec3 offset = new Vec3(-0.2, (double)0.0F, 0.3);
