...
public class UnwaveringNature extends BaseSporeEnchantment {
   ...
   public void doPostAttack(LivingEntity livingEntity, Entity entity, int value) {
      if (entity instanceof LivingEntity livingEntity1) {
         livingEntity1.addEffect(new MobEffectInstance((MobEffect)Seffects.MYCELIUM.get(), 600, 1));
      }
   ...
   public void doPostHurt(LivingEntity livingEntity, Entity entity, int value) {
      livingEntity.getArmorSlots().forEach((stack) -> {
         if (stack.getEnchantmentLevel(this) > 0) {
            if (!livingEntity.hasEffect((MobEffect)Seffects.MYCELIUM.get())) {
               livingEntity.addEffect(new MobEffectInstance((MobEffect)Seffects.MYCELIUM.get(), 600, 0));
            }

            if (Math.random() < 0.05) {
               ScentEntity scent = new ScentEntity((EntityType)Sentities.SCENT.get(), livingEntity.level());
