...
public class BuffAlliesGoal extends Goal {
   ...
   public BuffAlliesGoal(RangedBuff mob1, Class<? extends Mob> partnerClass, double speedModifier, int attackTime, int attackTime1, float attackRadius) {
      this(mob1, partnerClass, speedModifier, attackTime, attackTime1, attackRadius, (Predicate)null);
   }
   ...
   private Mob getFreePartner() {
      List<? extends Mob> list = this.level.getNearbyEntities(this.partnerClass, this.PARTNER_TARGETING, this.mob, this.mob.getBoundingBox().inflate(this.mob.getAttributeBaseValue(Attributes.FOLLOW_RANGE)));
