...
public interface CasingGenerator {
   ...
   private BlockState randomCasingBlock(RandomSource source) {
      ...
      return this.withCasingLight((BlockState)blocks.get(source.nextInt(blocks.size())), source);
   }

   private boolean isPossibleBlock(BlockState state) {
      if (state.hasProperty(CasingBiomassBlock.LIT)) {
         state = (BlockState)state.setValue(CasingBiomassBlock.LIT, false);
      }
               ...
               if ((Math.abs(i) != 2 || Math.abs(j) != 2 || Math.abs(k) != 2) && distance > (double)radius - (double)thickness / (double)2.0F && distance < (double)radius + (double)thickness / (double)2.0F) {
                  ...
                  if (Math.random() < 0.1 && !blockstate.isSolidRender(level, blockpos) && !blockstate.is(Blocks.BARRIER) && this.compare(level, blockpos) && !level.isClientSide) {
                     ...
                     if (Math.random() < 0.001) {
                        this.createSpots(blockpos, level, randomSource.nextInt(3, 6), Math.random() < (double)0.5F ? ((Block)Sblocks.SICKEN_BIOMASS_BLOCK.get()).defaultBlockState() : ((Block)Sblocks.CALCIFIED_BIOMASS_BLOCK.get()).defaultBlockState(), randomSource);
                     }

                     if (Math.random() < 0.001) {
                        this.createPussSpots(blockpos, level, randomSource.nextInt(2, 5), randomSource);
                     }
         ...
         for(int j = 0; j <= 2 * range; ++j) {
            for(int k = 0; k <= 2 * range; ++k) {
               ...
               if ((Math.abs(i) != 2 || Math.abs(j) != 2 || Math.abs(k) != 2) && distance < (double)range + (double)0.5F) {
                  ...
                  if (this.isPossibleBlock(level.getBlockState(blockpos))) {
                     level.setBlock(blockpos, this.withCasingLight(state, source), 3);
                  }
               }
            }
         }
               ...
               if (Math.abs(i) != 2 || Math.abs(j) != 2 || Math.abs(k) != 2) {
                  if (distance < (double)range + (double)0.5F && distance > (double)range - (double)0.5F) {
                     ...
                     if (level.getBlockState(blockpos).isAir()) {
                        level.setBlock(blockpos, this.withCasingLight(Math.random() < (double)0.5F ? ((Block)Sblocks.GASTRIC_BIOMASS.get()).defaultBlockState() : ((Block)Sblocks.BIOMASS_BLOCK.get()).defaultBlockState(), source), 3);
                     }
                  }
                  ...
                  if (distance < (double)range - (double)0.5F) {
                     BlockPos blockpos = pos.offset(i - range, j - range, k - range);
                     if (level.getBlockState(blockpos).isAir()) {
                        level.setBlock(blockpos, this.withCasingLight(Math.random() < (double)0.5F ? ((Block)Sblocks.SICKEN_BIOMASS_BLOCK.get()).defaultBlockState() : ((LiquidBlock)Sblocks.BILE.get()).defaultBlockState(), source), 3);
                     }
                  }
               }
            }
