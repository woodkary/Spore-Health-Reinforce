...
public class InfectedHuman extends Infected implements EvolvingInfected {
   public InfectedHuman(Level level) {
      super((EntityType)Sentities.INF_HUMAN.get(), level);
   }
   ...
   public List<String> getDropList() {
      return (List)SConfig.DATAGEN.inf_human_loot.get();
   }

   public static AttributeSupplier.Builder createAttributes() {
      return Mob.createMobAttributes().add(Attributes.MAX_HEALTH, (Double)SConfig.SERVER.inf_human_hp.get() * (Double)SConfig.SERVER.global_health.get()).add(Attributes.MOVEMENT_SPEED, 0.2).add(Attributes.ATTACK_DAMAGE, (Double)SConfig.SERVER.inf_human_damage.get() * (Double)SConfig.SERVER.global_damage.get()).add(Attributes.ARMOR, (Double)SConfig.SERVER.inf_human_armor.get() * (Double)SConfig.SERVER.global_armor.get()).add(Attributes.FOLLOW_RANGE, (double)16.0F).add(Attributes.ATTACK_KNOCKBACK, 0.3);
   }

   public void baseTick() {
      super.baseTick();
      this.tickEvolution(this, (List)SConfig.SERVER.human_ev.get(), ScamperVariants.DEFAULT);
   }
