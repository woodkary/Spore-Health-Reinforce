...
public class CalamityMovementControl extends MoveControl {
   ...
   private void tickWaterMoveTo() {
      ...
      if (Math.abs(d1) > (double)1.0E-5F) {
         this.mob.setYya(d1 > (double)0.0F ? speed : -speed);
      } else {
         this.mob.setYya(0.0F);
      }
      ...
      if (horizontalDirection != Vec3.ZERO) {
         ...
         double acceleration = Mth.clamp((double)speed * 0.35D, 0.035D, 0.16D);
         deltaMovement = deltaMovement.add(horizontalDirection.x * acceleration, (double)0.0F, horizontalDirection.z * acceleration);
         double maxHorizontalSpeed = Mth.clamp((double)speed * 1.25D + 0.04D, 0.12D, 0.35D);
         deltaMovement = this.limitHorizontalSpeed(deltaMovement, maxHorizontalSpeed);
      }

      double verticalAcceleration = Mth.clamp(d1 * 0.04D, -0.04D, 0.08D);
      if (this.mob.horizontalCollision && this.wantedY > this.mob.getY()) {
         verticalAcceleration += 0.08D;
      ...
      deltaMovement = new Vec3(deltaMovement.x, Mth.clamp(deltaMovement.y, -0.2D, 0.2D), deltaMovement.z);
   ...
   private Vec3 getWaterSlideDirection(Vec3 forward) {
      ...
      double probeDistance = Math.max((double)this.mob.getBbWidth() * 0.35D, 0.4D);
      ...
      if (leftClear != rightClear) {
         side = leftClear ? left : right;
      } else {
         side = this.mob.tickCount / 20 % 2 == 0 ? left : right;
      }
