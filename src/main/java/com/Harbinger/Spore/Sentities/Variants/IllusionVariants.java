...
public enum IllusionVariants {
   ...
   private static final IllusionVariants[] BY_ID = (IllusionVariants[])Arrays.stream(values()).sorted(Comparator.comparingInt(IllusionVariants::getVariant)).toArray((x$0) -> new IllusionVariants[x$0]);

   private IllusionVariants(int variant, String entity) {
   ...
   // $FF: synthetic method
   private static IllusionVariants[] $values() {
      return new IllusionVariants[]{KNIGHT, BRAIO, EVOKER, VOLATILE, VINDI, SCAMPER, SLASHER, LEAPER, WENDIGO, GRIEFER, GASTGABER, BROTKATZE, INQUISITOR, CREEPER};
   }
}
