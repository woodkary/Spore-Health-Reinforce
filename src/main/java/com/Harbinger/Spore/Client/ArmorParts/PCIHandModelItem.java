...
public class PCIHandModelItem extends ComplexHandModelItem {
   ...
   public PCIHandModelItem(InteractionHand slot, EntityModel model, ModelPart part, float x, float y, float z, float expand, float xspin, float yspin, float zspin, ResourceLocation glowLocation) {
      super(slot, (Item)Sitems.PCI.get(), model, part, x, y, z, expand, xspin, yspin, zspin);
   ...
   protected void renderModel(PoseStack poseStack, VertexConsumer consumer, int light, MultiBufferSource source, ItemStack stack, int color) {
      applyTransformEx(poseStack, this.x, this.y, this.z, this.expand, this.Xspin, this.Yspin, this.Zspin, () -> {
         this.part.render(poseStack, consumer, light, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
         VertexConsumer vertexConsumer = this.displayglow(source, stack);
         if (vertexConsumer != null) {
            this.part.render(poseStack, vertexConsumer, light, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
         }
   ...
   public VertexConsumer displayglow(MultiBufferSource source, ItemStack stack) {
      ...
      if (var4 instanceof PCI pci) {
         if (pci.getCharge(stack) > 0) {
            return source.getBuffer(RenderType.eyes(this.glowLocation));
         }
      }
