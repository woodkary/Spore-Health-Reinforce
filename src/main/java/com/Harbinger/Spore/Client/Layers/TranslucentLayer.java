...
public abstract class TranslucentLayer<T extends LivingEntity> extends EntityRenderLayer<T> {
   public TranslucentLayer(RenderLayerParent<T, net.minecraft.client.model.EntityModel<T>> p_117346_) {
      super(p_117346_);
   }
   ...
   public void render(PoseStack stack, MultiBufferSource bufferSource, int value, T type, float v1, float v2, float v3, float v4, float v5, float v6) {
      if (!type.isInvisible()) {
         VertexConsumer vertexconsumer = bufferSource.getBuffer(RenderType.entityTranslucent(this.getTexture(type)));
