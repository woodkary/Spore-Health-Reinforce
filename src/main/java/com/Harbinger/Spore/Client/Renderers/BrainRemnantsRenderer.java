...
public class BrainRemnantsRenderer extends BaseBlockEntityRenderer<BrainRemnantBlockEntity> {
   private BlockEntityModel brainModel = new BrainTentacleModel();
   private static final ResourceLocation TEXTURE = new ResourceLocation("spore", "textures/block/brain_bits.png");
   private static final ResourceLocation ROASTED_TEXTURE = new ResourceLocation("spore", "textures/block/roasted_brain_bits.png");
   private static final ResourceLocation TENTACLE = new ResourceLocation("spore", "textures/block/tentacle_brain.png");
   ...
   public void render(@NotNull BrainRemnantBlockEntity blockEntity, float partialTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
      ...
      if (blockEntity.isOnFire()) {
         this.renderTentacles(blockEntity, pPoseStack, partialTicks, pBuffer, pPackedLight, pPackedOverlay);
      }
   ...
   public void renderTentacles(@NotNull BrainRemnantBlockEntity blockEntity, PoseStack pPoseStack, float partialTicks, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
      ...
      this.brainModel.renderToBuffer(pPoseStack, vertexConsumer, pPackedLight, pPackedOverlay, 1.0F, 1.0F, 1.0F, 1.0F);
      pPoseStack.popPose();
   }
}
