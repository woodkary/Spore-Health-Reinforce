...
import com.Harbinger.Spore.Client.Models.SiegerLeg;
...
public class TentacleRenderer extends MobRenderer<Tentacle, EntityModel<Tentacle>> {
   private final TentacleSegmentModel tentacleModel2 = new TentacleSegmentModel();
   private final TentacleSegmentModel2 tentacleModel1 = new TentacleSegmentModel2();
   private final TentacleSegmentModel3 tentacleModel3 = new TentacleSegmentModel3();
   ...
   public EntityModel getProperModel(int val) {
      switch (val) {
         case 1 -> {
   ...
   private void renderTentacle(PoseStack stack, MultiBufferSource buffer, PartEntity[] segments, LivingEntity parent, float partial) {
      if (segments != null && segments.length >= 2) {
         ...
         for(int i = 0; i < segments.length; ++i) {
            ...
            this.renderConnection(entity, currentPos, stack, buffer, i, partial, red, green, blue);
            entity = currentPos;
         }
   ...
   public Vec3 processPosition(Entity entity, float partialTicks) {
      ...
      double centerY = interpolatedPos.y + (double)(entity.getBbHeight() / 2.0F);
   ...
   private void renderConnection(Entity from, Entity to, PoseStack stack, MultiBufferSource buffer, int index, float partial, float r, float g, float b) {
      if (from != null && to != null) {
         ...
         Vec3 direction = this.processPosition(to, partial).subtract(goingTo);
         ...
         if (!(length < 1.0E-4F)) {
            ...
            stack.scale(1.0F, length * 1.05F, 1.0F);
            model.setupAnim(from, 0.0F, 0.0F, (float)from.tickCount + partial, 0.0F, 0.0F);
            model.renderToBuffer(stack, consumer, 15728880, OverlayTexture.NO_OVERLAY, r, g, b, 1.0F);
            stack.popPose();
            stack.popPose();
         }
      }
