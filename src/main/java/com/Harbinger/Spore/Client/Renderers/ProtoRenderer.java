...
@OnlyIn(Dist.CLIENT)
public class ProtoRenderer extends OrganoidMobRenderer<Proto> {
   private final EntityModel normalProto = this.getModel();
   private final EntityModel nunny;
   private static final ResourceLocation TEXTURE = new ResourceLocation("spore", "textures/entity/proto_remade.png");
   private static final ResourceLocation OLD_PROTO = new ResourceLocation("spore", "textures/entity/proto.png");

   public ProtoRenderer(EntityRendererProvider.Context context) {
      super(context, new ProtoRedesign(context.bakeLayer(ProtoRedesign.LAYER_LOCATION)), 3.0F);
      this.nunny = new ProtoHivemindModel(context.bakeLayer(ProtoHivemindModel.LAYER_LOCATION));
   ...
   public void render(Proto type, float value1, float value2, PoseStack stack, MultiBufferSource bufferSource, int value3) {
      this.model = type.isNunny() ? this.nunny : this.normalProto;
