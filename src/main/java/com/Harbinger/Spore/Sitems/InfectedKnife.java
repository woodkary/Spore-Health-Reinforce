...
public class InfectedKnife extends SporeSwordBase implements LootModifierWeapon {
   public InfectedKnife() {
      super((double)(Integer)SConfig.SERVER.knife_damage.get(), (double)0.0F, (double)-1.0F, (Integer)SConfig.SERVER.knife_durability.get(), "knife");
   }

   public boolean canApplyAtEnchantingTable(ItemStack stack, Enchantment enchantment) {
      if (ModList.get().isLoaded("farmersdelight")) {
         Enchantment enchantment1 = (Enchantment)ForgeRegistries.ENCHANTMENTS.getValue(new ResourceLocation("farmersdelight:backstabbing"));
         if (enchantment1 != null) {
            return (super.canApplyAtEnchantingTable(stack, enchantment) || ImmutableSet.of(enchantment1).contains(enchantment) || ImmutableSet.of(Enchantments.LOYALTY).contains(enchantment)) && !ImmutableSet.of(Enchantments.MOB_LOOTING).contains(enchantment);
         }
      }
   ...
   public void releaseUsing(ItemStack stack, Level level, LivingEntity entity, int T) {
      if (entity instanceof Player player) {
         ...
         if (i >= 10 && !level.isClientSide) {
            ...
            ThrownKnife thrownSpear = new ThrownKnife(level, player, stack, this.getVariant(stack).getColor());
