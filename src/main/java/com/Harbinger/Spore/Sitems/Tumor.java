...
public class Tumor extends BaseItem {
   ...
   public InteractionResultHolder use(Level level, Player player, InteractionHand hand) {
      ...
      level.playSound((Player)null, player.getX(), player.getY(), player.getZ(), SoundEvents.EGG_THROW, SoundSource.PLAYERS, 0.5F, 0.4F / (player.getRandom().nextFloat() * 0.4F + 0.8F));
      if (!level.isClientSide) {
         ThrownTumor tumor = new ThrownTumor(level, player);
         tumor.setItem(itemstack);
         tumor.setType(this.type.getType());
   ...
   public static enum TumorType {
      ...
      // $FF: synthetic method
      private static TumorType[] $values() {
         return new TumorType[]{REGULAR, SICKEN, CALCIFIED, FROZEN, BILE};
      }
   }
}
